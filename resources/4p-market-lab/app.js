...
const ROLES = {
  customerExam: {
    ...
    greeting: "我最近在准备考试，下午最容易困。你可以问我什么时候会买、为什么会犹豫。",
  },
  customerHealth: {
    label: "健康型学生",
    boundary: "关注成分、负担和日常饮用，不会替团队评价完整营销方案。",
    greeting: "我会看配料和咖啡因，也不想为了提神影响晚上睡觉。",
  },
  customerSocial: {
    label: "社交尝新人群",
    boundary: "关注口味、包装和分享场景，不提供市场规模或销量数字。",
    greeting: "新品如果有记忆点，我愿意和朋友一起试，但我不喜欢太像功能饮料。",
  },
  channel: {
    label: "校园渠道商",
...
const DEFAULT_DECISION = {
  ...
  promotion: { sampling: 20, communities: 20, shortVideo: 20 },
  expectation: "",
  reason: "",
};
...
function loadState() {
  try {
    ...
    if (parsed?.version === 1 && parsed.sessionId) return parsed;
  } catch {}
...
function resultMarkup(result, roundLabel) {
  return `
    ...
    <h3>主要影响因素</h3>
    <ul class="factor-list">${result.factors.map((factor) => `<li>${escapeHtml(factor)}</li>`).join("")}</ul>
  `;
}
...
function renderBrief() {
  ...
  workspace.innerHTML = `
    <div class="stage-intro">
      ...
      <h2>让一款新品在校园里活下来</h2>
      <p>你们有 800 罐首批库存，需要决定卖给谁、做什么产品表达、定什么价格、在哪里卖、如何推广。没有随机事件，也没有隐藏骰子。</p>
    </div>
    <div class="scenario-grid">
      ...
      <article class="scenario-card"><h3>统一约束</h3><p>渠道预算 40 点、推广预算 60 点；单罐制造成本 ¥3.2，首批库存 800 罐。</p></article>
    </div>
...
function renderResearch() {
  ...
  workspace.innerHTML = `
    ...
    <div class="callout">建议问法：“什么情况下你会买？”“你最担心什么？”“什么会让你拒绝上架？”</div>
  ...
  return `
    <form class="decision-form" id="decision-form" data-round="${round}">
      <div class="form-row">
        ...
        <div class="field"><label for="product">Product · 产品表达</label><select id="product" name="product">
          <option value="focus" ${decision.product === "focus" ? "selected" : ""}>强效提神</option>
          <option value="balanced" ${decision.product === "balanced" ? "selected" : ""}>均衡轻负担</option>
          <option value="social" ${decision.product === "social" ? "selected" : ""}>清爽社交</option>
        </select></div>
      ...
      <section class="allocation-group">
        <div class="allocation-header"><h3>Promotion · 推广预算</h3><span class="budget-total" id="promotion-total">${Object.values(decision.promotion).reduce((a,b) => a+b, 0)} / 60</span></div>
        <div class="allocation-inputs">
          <div class="field"><label for="sampling">试饮活动</label><input id="sampling" name="sampling" type="number" min="0" max="60" value="${decision.promotion.sampling}" /></div>
          <div class="field"><label for="communities">学生社群合作</label><input id="communities" name="communities" type="number" min="0" max="60" value="${decision.promotion.communities}" /></div>
          <div class="field"><label for="shortVideo">校园短视频</label><input id="shortVideo" name="shortVideo" type="number" min="0" max="60" value="${decision.promotion.shortVideo}" /></div>
        </div>
      </section>

      <div class="field"><label for="expectation">${isRound2 ? "修改理由：你依据了什么证据？" : "结果预期：你认为这套策略会带来什么？"}</label><textarea id="expectation" name="expectation" rows="4" minlength="12" required>${escapeHtml(isRound2 ? decision.reason : decision.expectation)}</textarea><small>至少 12 个字。必须先写判断，再查看结果。</small></div>
...
function readDecisionForm(form, round) {
  ...
  const base = {
    ...
    promotion: { sampling: Number(data.get("sampling")), communities: Number(data.get("communities")), shortVideo: Number(data.get("shortVideo")) },
    expectation: round === "round1" ? String(data.get("expectation")).trim() : state.round1.expectation,
    reason: round === "round2" ? String(data.get("expectation")).trim() : "",
  };
  return base;
}

function decisionsDiffer(a, b) {
  return JSON.stringify({ ...a, expectation: "", reason: "" }) !== JSON.stringify({ ...b, expectation: "", reason: "" });
}
...
function wireDecisionForm(round) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if ((round === "round2" ? decision.reason : decision.expectation).length < 12) {
      error.textContent = "请先写下至少 12 个字的判断或修改理由。";
      return;
    }
    if (round === "round2" && !decisionsDiffer(state.round1, decision)) {
...
function renderReflection() {
  ...
  document.querySelector("#reflection-form").addEventListener("submit", (event) => {
    ...
    state.round2 = { ...clone(state.round1), reason: "" };
    saveState();
    setStage(5);
  });
}
...
function renderRound2() {
  workspace.innerHTML = `<div class="stage-intro"><p class="eyebrow">STEP 6 · 第二轮修改</p><h2>只改有证据支持的地方</h2><p>第二轮必须至少修改一个 4P 决策，并说明依据。不是所有数字都需要一起变。</p></div><div class="callout"><strong>你对第一轮的解释：</strong>${escapeHtml(state.reflection)}</div>${decisionFormMarkup("round2")}`;
...
function renderComparison() {
  workspace.innerHTML = `
    ...
    <div class="callout"><strong>第二轮修改依据：</strong>${escapeHtml(state.round2.reason)}</div>
    <h3>最后，请评价这次内部体验</h3>
...
function fallbackReply(role, message) {
  const replies = {
    customerExam: "我最常在图书馆出来、赶下一节课时买饮料。6 元比较容易顺手买；如果提神太强，我会担心晚上睡不着。",
    customerHealth: "无糖只是第一步，我还会看咖啡因标识是否清楚。便利店里能看清成分，我比在售货机上更安心。",
    customerSocial: "我会被口味、包装和朋友推荐吸引。价格高一点不是绝对问题，但它得值得拍照或一起尝试。",
